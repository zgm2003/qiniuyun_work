...
P3 是真问题。当前项目已经能把 3 章以上小说转换成 YAML 剧本，但项目资产仍只存在浏览器 localStorage。上线后只靠 localStorage 会丢数据、不能跨设备、也无法为后续登录和权限隔离提供基础。

本阶段只做 MySQL 基础持久化，不做 Redis、Auth、RBAC、管理端，也不替换 localStorage 草稿。
...
Prisma 后续可用，但现在会引入 schema、generate、migration、client 生命周期等额外概念。项目还没登录体系，数据模型也很小。现在上 Prisma 不解决真实问题。
