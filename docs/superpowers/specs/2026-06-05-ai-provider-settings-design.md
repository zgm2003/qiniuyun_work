...
【复杂度检查】
本阶段只支持 `openai-compatible` driver。第一版不做多租户 Key、不做计费、不做复杂供应商策略、不做完整管理端 UI、不做 RBAC。管理端 CRUD 和权限入口留到 P7；P6 只提供 server-only 的保存、读取、解密、模型刷新和健康检查边界，避免在没有权限体系时暴露敏感写接口。
...
- P7 管理端可以直接复用 server-only 服务，不需要重写运行时调用链。
...
### 不选方案：P6 同时做完整管理端和 RBAC

这会把 Key 加密、供应商运行时、权限、页面、表单、导航全部混成一个大 PR。没有好品味。P6 先把数据结构和运行时边界做正确，P7 再接管理端入口。
...
P6 可新增 server-only 服务 `refreshAIProviderModels`，但不新增没有权限保护的公开刷新接口。等 P7 有 `admin/member` 权限边界后，再把该服务接到管理端。
...
值得做。当前特殊情况是：生产 AI 配置靠 env，后续又想管理端维护 Key 和模型。如果继续堆 if，只会把 provider 调用、密钥、模型列表、健康检查混在一起。P6 的好品味做法是先把数据结构立住：provider、model、encrypted secret、runtime config resolver。这样代码路径短，兼容旧部署，后续 P7 管理端也有稳定落点。
